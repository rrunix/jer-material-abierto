# Cliente WebSockets con JavaScript {#sec-ws-client}
...
```javascript
new WebSocket('ws://IP:PUERTO/RUTA');
```
...
Este evento se dispara cuando algo sale mal: no se puede establecer la conexión con el servidor, hay un problema de red, o el servidor rechaza la conexión. Es importante manejar estos errores para informar al usuario de que algo no funciona correctamente. Hay que tener en cuenta que, por seguridad, el evento `error` no incluye detalles del fallo; el diagnóstico real se hace con `event.code` en `onclose`:
...
// Reasigna a la nueva conexión los listeners vistos en este capítulo
function configurarListeners(connection) {
  connection.onopen = function() { /* ... */ };
  connection.onmessage = function(msg) { /* ... */ };
  connection.onerror = function(e) { /* ... */ };
  connection.onclose = function(event) { /* ... */ };
...
En una aplicación real conviene además aumentar progresivamente el tiempo de espera entre reintentos (backoff exponencial) para no saturar un servidor caído.