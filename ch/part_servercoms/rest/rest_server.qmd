...
Un middleware de logging básico registra cada petición que llega al servidor. Los middlewares se ejecutan en el orden en que se registran, por lo que el logging debe registrarse antes que las rutas. Es importante llamar a `next()` al final para que la petición continúe su flujo normal hacia el controlador apropiado:
...
Los middlewares de manejo de errores son especiales porque tienen cuatro parámetros en lugar de tres. Express los identifica automáticamente por esta firma y los invoca cuando ocurre un error. Deben colocarse después de todas las rutas para capturar cualquier error que no haya sido manejado. Hay que tener en cuenta que en Express 4 los errores lanzados dentro de controladores `async` no llegan automáticamente a este middleware: hay que capturarlos y llamar a `next(err)`; en Express 5 sí se propagan automáticamente:
...
El archivo `public/index.html` estará disponible en `http://localhost:8080/index.html` y `public/script.js` en `http://localhost:8080/script.js`. Además, `express.static` sirve `index.html` automáticamente también en la raíz `/`.