...
La función `fetch()` es la API estándar moderna de JavaScript para realizar peticiones HTTP (@sec-rest-intro). Está integrada en todos los navegadores modernos y utiliza promesas (Promises) para manejar operaciones asincrónicas de manera elegante.

Cuando trabajamos con `fetch()`, tenemos dos formas principales de manejar las respuestas asincrónicas. La primera opción es encadenar promesas con `.then()` para procesar la respuesta. Primero verificamos si la petición fue exitosa con `response.ok`, luego convertimos la respuesta a JSON, y finalmente procesamos los datos. Si algo falla en cualquier punto de la cadena, el `.catch()` captura el error.
...
fetch('https://api.game.com/players/charlie', {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ username: 'charlie', email: 'newemail@example.com', level: 5 })
})
    .then(response => response.json())
...
En el siguiente capítulo (@sec-rest-server) implementaremos el servidor al que llama este cliente.