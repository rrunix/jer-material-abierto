...
function calculateUDPChecksum(srcIP, dstIP, srcPort, dstPort, data) {
    ...
    // Special case: 0x0000 is transmitted as 0xFFFF (0 means "no checksum" in UDP/IPv4)
...
TCP (Transmission Control Protocol) es el protocolo de transporte más utilizado en Internet y representa el extremo opuesto a UDP en términos de garantías y complejidad (RFC 9293, originalmente RFC 793). TCP prioriza la confiabilidad y el orden de los datos sobre la velocidad pura, siendo fundamental para aplicaciones como navegadores web, correo electrónico, transferencia de archivos y cualquier servicio que requiera integridad absoluta de los datos. Las características principales de TCP son las siguientes:
...
- **PSH**: Solicita entrega inmediata al proceso de aplicación.
...
- Tres ACK duplicados: Pérdida no tan severa, se reducen la ventana de congestión y el umbral a la mitad y se continúa en congestion avoidance (fast recovery).
...
for rtt in range(rtt_count):
    ...
    if perdida:
        ...
        if fase_actual == "Slow Start":
            ...
            ventana_actual = min(ventana_actual * 2, umbral)  # Limitar crecimiento al umbral
...
plt.tight_layout()
plt.savefig('resources/tcp_congestion_window.png', format='png', bbox_inches='tight')
plt.close()
```
...
Pasando a ejemplos concretos, World of Warcraft es un ejemplo de juego implementado sobre TCP. Los hechizos y ataques necesitan entrega garantizada para mantener la consistencia, así como las actualizaciones del inventario y estado de las misiones. Generalmente los MMORPGs pueden soportar latencias de 100 a 200 ms.
