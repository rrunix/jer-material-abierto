...
- `<!DOCTYPE html>`: Declara que este es un documento HTML (la declaración estándar introducida con HTML5).
...
**2. Relleno (padding):** Es el espacio entre el contenido y el borde. El fondo del elemento se extiende también bajo el padding. Sirve para dar "respiración" al contenido.
...
Ejemplo: si un elemento tiene `width: 200px`, `padding: 10px`, `border: 2px solid`, el ancho visible será 224px (2 + 10 + 200 + 10 + 2).
...
- NO aceptan `width` ni `height` (excepto los elementos reemplazados como `<img>`)
...
El elemento se extrae completamente del flujo normal y se posiciona respecto al ancestro posicionado más cercano (cualquier elemento con `position` diferente de `static`). Si no hay ninguno, se posiciona respecto al bloque contenedor inicial (en la práctica, el viewport de la página).
...
```css
/* El elemento que es el primer hijo de su padre */
li:first-child {
    font-weight: bold;
}

/* El elemento que es el último hijo de su padre */
li:last-child {
    border-bottom: none;
}
