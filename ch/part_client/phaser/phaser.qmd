...
En el capítulo anterior (@sec-javascript) hemos aprendido los fundamentos de JavaScript, incluyendo su orientación a objetos, los conceptos de DOM y BOM, y cómo configurar un entorno de desarrollo moderno con Node.js y Webpack. Ahora vamos a aplicar todos estos conocimientos para crear videojuegos web utilizando Phaser 3, uno de los frameworks más populares y potentes para el desarrollo de juegos en navegadores.
...
Phaser 3 soporta dos sistemas de renderizado: **WebGL**, que es el preferido por defecto y permite gráficos más avanzados pero requiere mayor soporte del hardware, y **Canvas**, el más compatible entre navegadores y dispositivos, que se usa como respaldo. La elección entre uno u otro dependerá de las necesidades específicas del proyecto y del público objetivo.
...
**Repositorio GitHub**: [https://github.com/phaserjs/phaser](https://github.com/phaserjs/phaser)
...
Phaser utiliza el elemento HTML5 `<canvas>` para renderizar los gráficos del juego. Este elemento es una etiqueta de HTML5 que permite dibujar gráficos mediante JavaScript.

El canvas funciona como un lienzo en blanco donde podemos dibujar formas, imágenes y animaciones. A diferencia de un elemento `<img>`, el canvas no tiene atributos `src` ni `alt`, pero sí tiene `width` y `height` que definen el espacio de coordenadas (por defecto 300x150 píxeles).
...
- **`width` y `height`**: Dimensiones del canvas en píxeles. Definen el área de juego visible. En el ejemplo, 800x600 píxeles es un tamaño clásico que funciona bien en la mayoría de pantallas. Otras opciones comunes:
  ...
  - Se puede usar `scale` para hacer el juego responsive
...
- **`physics`**: Objeto de configuración del motor de físicas. Tiene dos propiedades principales:
  - **`default`**: Motor de físicas a usar (`'arcade'` o `'matter'`). Arcade es el más usado por su simplicidad y rendimiento.
...
function create() {
    ...
    plataforma.body.setAllowGravity(false);  // No le afecta la gravedad global
...
3. **update()** → Se ejecuta continuamente (normalmente 60 veces por segundo, según la tasa de refresco del monitor) durante todo el juego
...
Phaser 3 incluye dos motores de físicas diferentes, cada uno diseñado para diferentes niveles de complejidad y casos de uso específicos. La elección del motor adecuado dependerá del tipo de juego que estemos desarrollando y del nivel de realismo físico que necesitemos.
...
En todos los ejemplos usaremos el motor Arcade; con el otro motor podría ser necesario hacer cambios.

### Matter Physics

Es el motor más avanzado y realista de los dos.
...
La interacción del jugador es el corazón de cualquier videojuego. En el capítulo de JavaScript (@sec-javascript) conocimos el DOM del navegador, cuyo sistema de eventos permite detectar clics y pulsaciones de teclas. Phaser 3 construye sobre estos conceptos y proporciona un sistema de entrada (input) mucho más potente y especializado para videojuegos, que simplifica la detección de teclado, ratón, táctil y gamepad.
...
Propiedades útiles: `touching.down`, `touching.up`, `touching.left`, `touching.right`, `blocked.down`, `blocked.up`. La diferencia entre `touching` y `blocked` es que `touching` detecta contacto con cualquier objeto, mientras que `blocked` solo detecta contacto con objetos inmóviles y con los límites del mundo; para saltos robustos conviene comprobar `body.blocked.down || body.touching.down`. Estas propiedades son esenciales para controlar mecánicas como saltos o detectar si un personaje está contra una pared.
